...
fun SignUpScreen(
    ...
    if (viewModel.signUpDataState.isLoading) {
        focusManager.clearFocus()
        LoadingAnimation(modifier = Modifier.fillMaxSize())
    }

    if (viewModel.signUpDataState.signUpSuccess) {
        viewModel.startingStateLoggedIn()
        Toast.makeText(LocalContext.current, "Sign Un Successful", Toast.LENGTH_SHORT).show()
