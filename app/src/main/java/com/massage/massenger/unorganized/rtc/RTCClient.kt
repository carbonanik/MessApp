...
import com.massage.massenger.common.PeerLeft
...
class RTCClient(
        ...
        val sendMessage: (RTCMessage) -> Unit
) {
...
//    val db = Firebase.firestore
    ...
    private fun PeerConnection.answer(sdpObserver: SdpObserver) {
        ...
        createAnswer(object : SdpObserver by sdpObserver {
            override fun onCreateSuccess(desc: SessionDescription?) {
                val sdpAnswer = desc?.let {
                    SDPMessage(
                        it.type,
                        it.description
                    )
                }
                // todo send sdp answer
                sdpAnswer?.let { sendMessage(it) }
...
//                }
        ...
        val peerLeft = PeerLeft()
        sendMessage(peerLeft)
